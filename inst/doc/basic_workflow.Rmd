...
Classification of Diseases (ICD) codes. The exact ICD version
depends on the date of the report.

> If you wish to perform a full search of indications, you must
enclose terms from both MedDRA and ICD.

You can add the indication columns to `demo` or other tables
(`drug`, `adr`, `link`, or even `ind`) with `add_ind()`.
...
As all drugs from all reports do not appear in the ind table, there might be
missing data in the columns created with `add_ind()` (which couldn't happen 
with `add_drug()` or `add_adr()`.

Thus, you can't use `check_dm()`, which doesn't work in case of missing data.
`desc_facvar()` will do the job instead.
